...
tenant_admin_id=$(get_id keystone tenant-create --name admin --description "Admin Tenant")
user_admin_id=$(get_id keystone user-create --name admin --pass $USER_ADMIN_PASS)
role_admin_id=$(get_id keystone role-create --name admin)
keystone user-role-add --user $user_admin_id --tenant_id $tenant_admin_id --role $role_admin_id

tenant_demo_id=$(get_id keystone tenant-create --name demo --description "Demo Tenant")
user_demo_id=$(get_id keystone user-create --name demo --pass $USER_DEMO_PASS)
# After keystone is installed, there already exists a 'role' named '_member_'
role_default_id=$(get_id keystone role-create --name $ROLE_DEFAULT)
keystone user-role-add --user $user_demo_id --tenant_id $tenant_demo_id --role $role_default_id
